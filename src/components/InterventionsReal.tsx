...
// Import des hooks et types pour l'API réelle
import { useInterventions, useStats, useApiConnection, useInterventionMutations } from '@/hooks/useRealApi';
import { RealIntervention } from '@/services/realApi';
import { useDebounce } from '@/hooks/useDebounce';
import { adaptRealInterventionToIntervention } from '@/utils/interventionAdapter';

export const InterventionsReal: React.FC = () => {
  const [selectedIntervention, setSelectedIntervention] = useState<RealIntervention | null>(null);
  ...
  // Hooks pour l'API réelle
  const { isConnected, loading: connectionLoading } = useApiConnection();
  const { data: statsData } = useStats();
  const { createIntervention, updateIntervention, deleteIntervention } = useInterventionMutations();

  // Fonction pour obtenir l'ID utilisateur par nom
  const getUserIdByName = (userName: string) => {
    // Mapping des noms vers les IDs (basé sur la base de données)
  ...
  const interventionParams = {
    page: selectedUser ? 1 : currentPage, // Si utilisateur sélectionné, toujours page 1
    limit: selectedUser ? 10000 : 100, // Si utilisateur sélectionné, récupérer toutes ses interventions
    status: selectedUser ? undefined : selectedStatus || undefined, // Ne pas filtrer par statut si utilisateur sélectionné
    user_id: selectedUser ? getUserIdByName(selectedUser) : undefined,
  ...
  const { 
    ...
    pagination,
    refetch 
  } = useInterventions(interventionParams);
  ...
  const filteredInterventions = allInterventions.filter(intervention => {
    // Filtre par utilisateur (si pas déjà filtré par l'API)
    if (!selectedUser && intervention.utilisateur_assigne !== selectedUser) {
      return false;
    }
    
    // Filtre par statut d'intervention (côté frontend si utilisateur sélectionné)
    ...
    if (selectedArtisanStatuses.length > 0 && intervention.artisan_status) {
      if (!selectedArtisanStatuses.includes(intervention.artisan_status as ARTISAN_STATUS)) {
        return false;
      }
    }
    ...
    if (selectedDossierStatuses.length > 0 && intervention.artisan_dossier_status) {
      if (!selectedDossierStatuses.includes(intervention.artisan_dossier_status as ARTISAN_DOSSIER_STATUS)) {
        return false;
      }
    }
  ...
  // Pagination côté frontend si utilisateur sélectionné
  const itemsPerPage = selectedUser ? 100 : 100;
  const totalFiltered = filteredInterventions.length;
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const interventions = selectedUser ? filteredInterventions.slice(startIndex, endIndex) : filteredInterventions;

  // Vérifier si une intervention est sélectionnée via l'URL
  useEffect(() => {
    const selectedId = searchParams.get('selected');
    if (selectedId && interventions.length > 0) {
      const intervention = interventions.find((i: RealIntervention) => i.id === selectedId);
      if (intervention) {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [selectedStatus, selectedUser, debouncedSearchTerm]);

  // Obtenir les listes uniques pour les filtres
  const uniqueUsers = Array.from(new Set(interventions.map(i => i.utilisateur_assigne).filter(Boolean)));
  ...
  const statusConfig = {
    ...
    annulee: { label: 'Annulé', icon: Clock, color: 'bg-red-100 text-red-700 border-red-200', activeColor: 'bg-red-500 text-white' },
    terminee: { label: 'Terminé', icon: CheckCircle, color: 'bg-emerald-100 text-emerald-700 border-emerald-200', activeColor: 'bg-emerald-500 text-white' },
    ...
    att_acompte: { label: 'Att Acompte', icon: Clock, color: 'bg-cyan-100 text-cyan-700 border-cyan-200', activeColor: 'bg-cyan-500 text-white' },
  ...
  // Obtenir les statuts disponibles selon le contexte (global ou par utilisateur)
  const getAvailableStatuses = () => {
    if (selectedUser) {
      // Si un utilisateur est sélectionné, utiliser les statuts de ses interventions
      const userInterventions = interventions.filter(intervention => 
        intervention.utilisateur_assigne === selectedUser
      );
      return Array.from(new Set(userInterventions.map(i => i.statut)));
    } else {
      // Sinon, utiliser les statistiques globales
      return statsData?.par_statut ? Object.keys(statsData.par_statut) : [];
    }
  };
  
  const allAvailableStatuses = getAvailableStatuses();
  
  // Statuts supplémentaires (disponibles dans le menu) - utiliser les stats contextuelles
  const additionalStatuses = allAvailableStatuses.filter(status => !defaultStatuses.includes(status));
  ...
  const getInterventionCountByStatus = (status: string) => {
    // Si un utilisateur est sélectionné, calculer les stats basées sur les interventions filtrées
    if (selectedUser) {
      const userInterventions = interventions.filter(intervention => 
        intervention.utilisateur_assigne === selectedUser
      );
      ...
      if (status === '') {
        return userInterventions.length;
      }
      return userInterventions.filter(intervention => intervention.statut === status).length;
    }
    
    // Sinon, utiliser les statistiques globales
    if (!statsData?.par_statut) {
      // Fallback sur les interventions locales si les stats ne sont pas disponibles
      if (status === '') {
    ...
    if (status === '') {
      ...
      return statsData.total || 0;
    }
    
    // Utiliser les statistiques globales de l'API
    return statsData.par_statut[status] || 0;
  };
  ...
  const sortedInterventions = [...filteredInterventions].sort((a, b) => {
    ...
    switch (sortConfig.field) {
      ...
      case 'marge':
        ...
        aValue = (a.montant || 0) - (a.cout_sst || 0) - (a.cout_materiaux || 0) - (a.cout_interventions || 0);
        bValue = (b.montant || 0) - (b.cout_sst || 0) - (b.cout_materiaux || 0) - (b.cout_interventions || 0);
        break;
      default:
        return 0;
    }
  ...
  const handleEditIntervention = (intervention: any) => {
    // Convertir RealIntervention vers Intervention pour la compatibilité
    const adaptedIntervention = adaptRealInterventionToIntervention(intervention);
    setSelectedIntervention(intervention);
  };

  const handleSendEmail = (intervention: any) => {
    console.log('Envoi email pour:', intervention.id);
    // Ici vous pouvez implémenter l'envoi d'email
  };

  const handleCall = (intervention: any) => {
    console.log('Appel pour:', intervention.id);
    // Ici vous pouvez implémenter l'appel
  };

  const handleAddDocument = (intervention: any) => {
    console.log('Ajout document pour:', intervention.id);
    // Ici vous pouvez implémenter l'ajout de document
  };

  const handleStatusChange = async (intervention: any, newStatus: string) => {
    try {
      const updatedIntervention = await updateIntervention(intervention.id, { statut: newStatus });
  ...
  const handleAmountChange = async (intervention: any, amount: number) => {
    try {
      const updatedIntervention = await updateIntervention(intervention.id, { montant: amount });
      refetch();
      if (selectedIntervention?.id === intervention.id) {
        setSelectedIntervention(updatedIntervention);
      }
  ...
  const handleDateChange = async (intervention: any, field: string, date: string) => {
    try {
      const updateData = field === 'echeance' ? { echeance: date } : { cree: date };
      const updatedIntervention = await updateIntervention(intervention.id, updateData);
      refetch();
      if (selectedIntervention?.id === intervention.id) {
        setSelectedIntervention(updatedIntervention);
      }
  ...
  const handleAddressChange = async (intervention: any, address: string) => {
    try {
      const updatedIntervention = await updateIntervention(intervention.id, { adresse: address });
      refetch();
      if (selectedIntervention?.id === intervention.id) {
        setSelectedIntervention(updatedIntervention);
      }
  ...
  const handleArtisanChange = async (intervention: any, artisan: string) => {
    try {
      const updatedIntervention = await updateIntervention(intervention.id, { artisan: artisan });
      refetch();
      if (selectedIntervention?.id === intervention.id) {
        setSelectedIntervention(updatedIntervention);
      }
  ...
  const handleArtisanStatusChange = async (intervention: any, newStatus: ARTISAN_STATUS) => {
    try {
      const updatedIntervention = await updateIntervention(intervention.id, { artisan_status: newStatus });
      refetch();
      if (selectedIntervention?.id === intervention.id) {
        setSelectedIntervention(updatedIntervention);
      }
  ...
  const handleArtisanDossierStatusChange = async (intervention: any, newStatus: ARTISAN_DOSSIER_STATUS) => {
    try {
      const updatedIntervention = await updateIntervention(intervention.id, { artisan_dossier_status: newStatus });
      refetch();
      if (selectedIntervention?.id === intervention.id) {
        setSelectedIntervention(updatedIntervention);
      }
  ...
  const handleClientChange = async (intervention: any, client: string) => {
    try {
      const updatedIntervention = await updateIntervention(intervention.id, { client: client });
      refetch();
      if (selectedIntervention?.id === intervention.id) {
        setSelectedIntervention(updatedIntervention);
      }
  ...
  const handleDescriptionChange = async (intervention: any, description: string) => {
    try {
      const updatedIntervention = await updateIntervention(intervention.id, { description: description });
      refetch();
      if (selectedIntervention?.id === intervention.id) {
        setSelectedIntervention(updatedIntervention);
      }
  ...
  const handleNotesChange = async (intervention: any, notes: string) => {
    try {
      const updatedIntervention = await updateIntervention(intervention.id, { notes: notes });
      refetch();
      if (selectedIntervention?.id === intervention.id) {
        setSelectedIntervention(updatedIntervention);
      }
  ...
  const handleCoutSSTChange = async (intervention: any, amount: number) => {
    try {
      const updatedIntervention = await updateIntervention(intervention.id, { cout_sst: amount });
      refetch();
      if (selectedIntervention?.id === intervention.id) {
        setSelectedIntervention(updatedIntervention);
      }
  ...
  const handleCoutMateriauxChange = async (intervention: any, amount: number) => {
    try {
      const updatedIntervention = await updateIntervention(intervention.id, { cout_materiaux: amount });
      refetch();
      if (selectedIntervention?.id === intervention.id) {
        setSelectedIntervention(updatedIntervention);
      }
  ...
  const handleCoutInterventionsChange = async (intervention: any, amount: number) => {
    try {
      const updatedIntervention = await updateIntervention(intervention.id, { cout_interventions: amount });
      refetch();
      if (selectedIntervention?.id === intervention.id) {
        setSelectedIntervention(updatedIntervention);
      }
  ...
  const handleUserChange = async (intervention: any, username: string) => {
    try {
      const updatedIntervention = await updateIntervention(intervention.id, { utilisateur_assigne: username });
      refetch();
      if (selectedIntervention?.id === intervention.id) {
        setSelectedIntervention(updatedIntervention);
      }
  ...
  // Afficher un message d'erreur si la connexion API échoue
        ...
        <Card className="border-red-200 bg-red-50">
          <CardContent className="p-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="font-semibold text-red-800">Erreur de connexion au backend</h3>
                <p className="text-red-600 text-sm">
                  Impossible de se connecter au serveur backend sur http://localhost:8001. 
  ...
  // Afficher un message de chargement si la connexion est en cours
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-blue-600">Connexion au backend en cours...</span>
            </div>
          </CardContent>
        </Card>
      </div>
    ...
    <div className="space-y-6" ref={containerRef}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm text-green-600">Connecté au backend</span>
        </div>
        {error && (
            ...
            >
              {sortedInterventions.map((intervention, index) => (
                ...
                >
                  ...
                  <InterventionCard
                    intervention={adaptRealInterventionToIntervention(intervention)}
                  ...
                  />
                     ...
                     {/* Pagination */}
           {!loading && (selectedUser ? totalFiltered : total) > 0 && (
             <div className="mt-6 flex items-center justify-between border-t pt-4">
               <div className="text-sm text-gray-600">
                 Affichage de {((currentPage - 1) * itemsPerPage) + 1} à {Math.min(currentPage * itemsPerPage, selectedUser ? totalFiltered : total)} sur {selectedUser ? totalFiltered : total} interventions
               </div>
               <div className="flex items-center space-x-2">
                 <Button
                   variant="outline"
                 ...
                 <div className="flex items-center space-x-1">
                   {Array.from({ length: Math.min(5, Math.ceil((selectedUser ? totalFiltered : total) / itemsPerPage)) }, (_, i) => {
                     const pageNum = i + 1;
                     return (
                       <Button
                         key={pageNum}
                   ...
                   {Math.ceil((selectedUser ? totalFiltered : total) / itemsPerPage) > 5 && (
                 ...
                 <Button
                   variant="outline"
                   size="sm"
                   onClick={() => setCurrentPage(Math.min(Math.ceil((selectedUser ? totalFiltered : total) / itemsPerPage), currentPage + 1))}
                   disabled={currentPage >= Math.ceil((selectedUser ? totalFiltered : total) / itemsPerPage)}
                 >
                   Suivant
                 </Button>
               </div>
      ...
      {selectedIntervention && (
        <div className="fixed right-6 top-24 bottom-6 w-96 z-50">
          <InterventionDetailCard
            intervention={selectedIntervention ? adaptRealInterventionToIntervention(selectedIntervention) : null}
