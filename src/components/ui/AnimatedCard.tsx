import React, { useState, useRef } from 'react';

interface AnimatedCardProps {
  onMouseEnter?: () => void;
  onMouseLeave?: () => void;
  statusColor?: string; // Couleur du statut pour synchroniser le background
}

const AnimatedCard: React.FC<AnimatedCardProps> = ({ onMouseEnter, onMouseLeave, statusColor = 'rgba(255, 255, 255, 0.91)' }) => {
  const [hoveredCard, setHoveredCard] = useState<number | null>(null);
  const [isContainerHovered, setIsContainerHovered] = useState(false);
  const containerRef = useRef<HTMLDivElement>(null);

  // Fonction pour ajouter la transparence à la couleur
  const getTransparentColor = (color: string) => {
    // Si c'est déjà une couleur rgba, on change juste l'alpha
    if (color.startsWith('rgba(')) {
      return color.replace(/[\d.]+\)$/, '0.15)');
    }
    // Si c'est une couleur hex, on la convertit en rgba avec alpha 0.15
    if (color.startsWith('#')) {
      const hex = color.replace('#', '');
      const r = parseInt(hex.substr(0, 2), 16);
      const g = parseInt(hex.substr(2, 2), 16);
      const b = parseInt(hex.substr(4, 2), 16);
      return `rgba(${r}, ${g}, ${b}, 0.15)`;
    }
    // Si c'est une couleur rgb, on ajoute l'alpha
    if (color.startsWith('rgb(')) {
      return color.replace('rgb(', 'rgba(').replace(')', ', 0.15)');
    }
    // Fallback
    return 'rgba(255, 255, 255, 0.15)';
  };

  const transparentColor = getTransparentColor(statusColor);

  // Fonction pour calculer le scale basé sur la distance de la carte survolée
  const getCardScale = (cardIndex: number) => {
    if (!isContainerHovered || hoveredCard === null) return 1; // Taille normale par défaut
    
    const distance = Math.abs(cardIndex - hoveredCard);
    
    if (distance === 0) return 1; // Carte survolée : taille normale
    if (distance === 1) return 0.85; // Cartes adjacentes : dézoom 15%
    if (distance === 2) return 0.7; // Cartes éloignées : dézoom 30%
    
    return 0.7; // Par défaut pour les cartes très éloignées
  };

  // Fonction pour calculer le z-index basé sur la distance
  const getCardZIndex = (cardIndex: number) => {
    if (!isContainerHovered || hoveredCard === null) return 1;
    
    const distance = Math.abs(cardIndex - hoveredCard);
    
    if (distance === 0) return 10; // Carte survolée au premier plan
    if (distance === 1) return 5; // Cartes adjacentes
    if (distance === 2) return 1; // Cartes éloignées
    
    return 1;
  };

  // Fonction pour calculer la rotation initiale
  const getInitialRotation = (cardIndex: number) => {
    if (!isContainerHovered) {
      // État initial avec rotations
      if (cardIndex === 0) return -15;
      if (cardIndex === 1) return 5;
      if (cardIndex === 2) return 25;
    }
    // Au hover : rotations à 0 pour alignement horizontal
    return 0;
  };

  // Fonction pour calculer le translateX lors du hover
  const getHoverTranslateX = (cardIndex: number) => {
    if (!isContainerHovered) return 0;
    
    if (cardIndex === 0) return -220; // Github
    if (cardIndex === 1) return -190; // Code
    if (cardIndex === 2) return -160; // Earn
    return 0;
  };

  const handleCardMouseEnter = (cardIndex: number) => {
    setHoveredCard(cardIndex);
  };

  const handleCardMouseLeave = () => {
    setHoveredCard(null);
  };

  // Gestionnaire pour l'entrée de la souris sur le conteneur
  const handleContainerMouseEnter = () => {
    setIsContainerHovered(true);
    setHoveredCard(2); // Par défaut, on se situe sur la carte Earn (index 2)
  };

  // Gestionnaire pour la sortie de la souris du conteneur
  const handleContainerMouseLeave = () => {
    setIsContainerHovered(false);
    setHoveredCard(null);
  };

  return (
    <div
      className="animated-stack"
      style={{
        position: 'absolute',
        display: 'flex',
        justifyContent: 'center',
        alignItems: 'center',
        transition: 'transform 200ms ease-out',
        zIndex: 9999
      }}
      onMouseEnter={handleContainerMouseEnter}
      onMouseLeave={handleContainerMouseLeave}
      ref={containerRef}
    >
      <div
        style={{
          position: 'relative',
          display: 'flex',
          justifyContent: 'center',
          alignItems: 'center'
        }}
      >
        <div
          data-text="Github"
          className="card card-1"
          style={{
            position: 'relative',
            width: '180px',
            height: '200px',
            background: `linear-gradient(${transparentColor}, transparent)`,
            border: '1px solid rgba(255, 255, 255, 0.1)',
            boxShadow: '0 25px 25px rgba(0, 0, 0, 0.15)',
            display: 'flex',
            justifyContent: 'center',
            alignItems: 'center',
            borderRadius: '40px',
            margin: '0 -45px',
            backdropFilter: 'blur(10px)',
            transition: 'all 200ms ease-out',
            willChange: 'transform, z-index',
            transform: `
              rotate(${getInitialRotation(0)}deg) 
              translateX(${getHoverTranslateX(0)}px) 
              translateY(${isContainerHovered ? 20 : 60}px) 
              scale(${getCardScale(0)})
            `,
            zIndex: getCardZIndex(0)
          }}
          onMouseEnter={() => handleCardMouseEnter(0)}
          onMouseLeave={handleCardMouseLeave}
        >
          <svg viewBox="0 0 496 512" height="1em" xmlns="http://www.w3.org/2000/svg">
            <path d="M165.9 397.4c0 2-2.3 3.6-5.2 3.6-3.3.3-5.6-1.3-5.6-3.6 0-2 2.3-3.6 5.2-3.6 3-.3 5.6 1.3 5.6 3.6zm-31.1-4.5c-.7 2 1.3 4.3 4.3 4.9 2.6 1 5.6 0 6.2-2s-1.3-4.3-4.3-5.2c-2.6-.7-5.5.3-6.2 2.3zm44.2-1.7c-2.9.7-4.9 2.6-4.6 4.9.3 2 2.9 3.3 5.9 2.6 2.9-.7 4.9-2.6 4.6-4.6-.3-1.9-3-3.2-5.9-2.9zM244.8 8C106.1 8 0 113.3 0 252c0 110.9 69.8 205.8 169.5 239.2 12.8 2.3 17.3-5.6 17.3-12.1 0-6.2-.3-40.4-.3-61.4 0 0-70 15-84.7-29.8 0 0-11.4-29.1-27.8-36.6 0 0-22.9-15.7 1.6-15.4 0 0 24.9 2 38.6 25.8 21.9 38.6 58.6 27.5 72.9 20.9 2.3-16 8.8-27.1 16-33.7-55.9-6.2-112.3-14.3-112.3-110.5 0-27.5 7.6-41.3 23.6-58.9-2.6-6.5-11.1-33.3 2.6-67.9 20.9-6.5 69 27 69 27 20-5.6 41.5-8.5 62.8-8.5s42.8 2.9 62.8 8.5c0 0 48.1-33.6 69-27 13.7 34.7 5.2 61.4 2.6 67.9 16 17.7 25.8 31.5 25.8 58.9 0 96.5-58.9 104.2-114.8 110.5 9.2 7.9 17 22.9 17 46.4 0 33.7-.3 75.4-.3 83.6 0 6.5 4.6 14.4 17.3 12.1C428.2 457.8 496 362.9 496 252 496 113.3 383.5 8 244.8 8zM97.2 352.9c-1.3 1-1 3.3.7 5.2 1.6 1.6 3.9 2.3 5.2 1 1.3-1 1-3.3-.7-5.2-1.6-1.6-3.9-2.3-5.2-1zm-10.8-8.1c-.7 1.3.3 2.9 2.3 3.9 1.6 1 3.6.7 4.3-.7.7-1.3-.3-2.9-2.3-3.9-2-.6-3.6-.3-4.3.7zm32.4 35.6c-1.6 1.3-1 4.3 1.3 6.2 2.3 2.3 5.2 2.6 6.5 1 1.3-1.3.7-4.3-1.3-6.2-2.2-2.3-5.2-2.6-6.5-1zm-11.4-14.7c-1.6 1-1.6 3.6 0 5.9 1.6 2.3 4.3 3.3 5.6 2.3 1.6-1.3 1.6-3.9 0-6.2-1.4-2.3-4-3.3-5.6-2z" />
          </svg>
        </div>
        <div
          data-text="Code"
          className="card card-2"
          style={{
            position: 'relative',
            width: '180px',
            height: '200px',
            background: `linear-gradient(${transparentColor}, transparent)`,
            border: '1px solid rgba(255, 255, 255, 0.1)',
            boxShadow: '0 25px 25px rgba(0, 0, 0, 0.15)',
            display: 'flex',
            justifyContent: 'center',
            alignItems: 'center',
            borderRadius: '40px',
            margin: '0 -45px',
            backdropFilter: 'blur(10px)',
            transition: 'all 200ms ease-out',
            willChange: 'transform, z-index',
            transform: `
              rotate(${getInitialRotation(1)}deg) 
              translateX(${getHoverTranslateX(1)}px) 
              translateY(${isContainerHovered ? 20 : 60}px) 
              scale(${getCardScale(1)})
            `,
            zIndex: getCardZIndex(1)
          }}
          onMouseEnter={() => handleCardMouseEnter(1)}
          onMouseLeave={handleCardMouseLeave}
        >
          <svg viewBox="0 0 640 512" height="1em" xmlns="http://www.w3.org/2000/svg">
            <path d="M392.8 1.2c-17-4.9-34.7 5-39.6 22l-128 448c-4.9 17 5 34.7 22 39.6s34.7-5 39.6-22l128-448c4.9-17-5-34.7-22-39.6zm80.6 120.1c-12.5 12.5-12.5 32.8 0 45.3L562.7 256l-89.4 89.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l112-112c12.5-12.5 12.5-32.8 0-45.3l-112-112c-12.5-12.5-32.8-12.5-45.3 0zm-306.7 0c-12.5-12.5-32.8-12.5-45.3 0l-112 112c-12.5 12.5-12.5 32.8 0 45.3l112 112c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L77.3 256l89.4-89.4c12.5-12.5 12.5-32.8 0-45.3z" />
          </svg>
        </div>
        <div
          data-text="Earn"
          className="card card-3"
          style={{
            position: 'relative',
            width: '180px',
            height: '200px',
            background: `linear-gradient(${transparentColor}, transparent)`,
            border: '1px solid rgba(255, 255, 255, 0.1)',
            boxShadow: '0 25px 25px rgba(0, 0, 0, 0.15)',
            display: 'flex',
            justifyContent: 'center',
            alignItems: 'center',
            borderRadius: '40px',
            margin: '0 -45px',
            backdropFilter: 'blur(10px)',
            transition: 'all 200ms ease-out',
            willChange: 'transform, z-index',
            transform: `
              rotate(${getInitialRotation(2)}deg) 
              translateX(${getHoverTranslateX(2)}px) 
              translateY(${isContainerHovered ? 20 : 60}px) 
              scale(${getCardScale(2)})
            `,
            zIndex: getCardZIndex(2)
          }}
          onMouseEnter={() => handleCardMouseEnter(2)}
          onMouseLeave={handleCardMouseLeave}
        >
          <svg viewBox="0 0 576 512" height="1em" xmlns="http://www.w3.org/2000/svg">
            <path d="M64 64C28.7 64 0 92.7 0 128V384c0 35.3 28.7 64 64 64H512c35.3 0 64-28.7 64-64V128c0-35.3-28.7-64-64-64H64zm64 320H64V320c35.3 0 64 28.7 64 64zM64 192V128h64c0 35.3-28.7 64-64 64zM448 384c0-35.3 28.7-64 64-64v64H448zm64-192c-35.3 0-64-28.7-64-64h64v64zM288 160a96 96 0 1 1 0 192 96 96 0 1 1 0-192z" />
          </svg>
        </div>
      </div>

      <style dangerouslySetInnerHTML={{
        __html: `
          .card::before {
            content: attr(data-text);
            position: absolute;
            bottom: 0;
            width: 100%;
            height: 40px;
            background: rgba(255, 255, 255, 0.05);
            display: flex;
            justify-content: center;
            align-items: center;
            color: #000;
            font-weight: 600;
          }
          
          .card svg {
            font-size: 2.5em;
            fill: #000;
          }
          
          /* État initial - scale 0.4 + rotations + descente de 60px */
          .animated-stack {
            transform: scale(0.4) translateY(60px) translateX(-40px);
          }
          
          /* Hover du conteneur → scale 0.6 + alignement horizontal + décalage global */
          .animated-stack:hover {
            transform: scale(0.6) translateX(0px) translateY(20px);
          }
        `
      }} />
    </div>
  );
};

export default AnimatedCard;
